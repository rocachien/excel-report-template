...
//
    ...
    Workbook.prototype.getSubstitutionWithNormal = function(substitutions, placeholder) {
    ...
    Workbook.prototype.substitute = function(sheetName, substitutions) {
        ...
        sheetData.findall("row").forEach(function(row) {
            ...
            // Fix error dont copy format cell when cell be merged - KL 27.6.2019
            if (newTableRows.length > 0) {
            ...
            substitution.forEach(function(element, idx) {
                ...
                if(key.length>0 && key[0]==='=') {
                    ...
                    for(let i =1;i<arr.length;i++){
                        ...
                        if (arr2.length>0)
                        {
                            arr2[0] = String(parseInt(arr2[0]) + upFormula);
                            newKey += arr2.join('');
                            continue;
                        };
                        newKey += arr[i];
